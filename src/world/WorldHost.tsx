...
export interface LandingMeadowState {
...
class LandingMeadowScene extends Phaser.Scene {
  ...
  private worldState: LandingMeadowState;
  private sequenceRunning = false;
  private onSequenceComplete: (sequence: 'first-light') => void;

  constructor(initialState: LandingMeadowState, onSequenceComplete: (sequence: 'first-light') => void) {
  ...
  create() {
    ...
    this.light = this.add.circle(0, 0, 10, 0xffe9a8, 0.9).setDepth(4);
    ...
    else if (this.worldState.firstLightRevealed) this.startLightPulse();
  }

  setWorldState(next: LandingMeadowState) {
    ...
    if (!this.sequenceRunning) {
      this.redrawFromState();
      if (next.firstLightRevealed) this.startLightPulse();
    }
  }

  private redrawFromState = () => {
    this.redraw(this.worldState.firstLightRevealed && !this.sequenceRunning);
  };

  private redraw(revealed: boolean) {
    ...
    g.fillStyle(0x759f98, 1);
    g.fillRoundedRect(w * 0.02, h * 0.63, w * 0.96, Math.max(18, h * 0.075), 30);
    g.fillStyle(revealed ? 0xb8ddd0 : 0xa7c8be, 1);
    ...
    this.light.setVisible(revealed || this.sequenceRunning);
    if (!this.sequenceRunning) this.light.setAlpha(revealed ? 0.72 : 0).setScale(1);
  }

  private startLightPulse() {
    if (!this.light || !this.worldState.firstLightRevealed || this.sequenceRunning) return;
  ...
  private spawnSparkles() {
    ...
    for (let i = 0; i < 10; i += 1) {
      ...
      const sparkle = this.add.circle(this.light.x, this.light.y, 2 + (i % 3), i % 2 ? 0xfff2b8 : 0xf5dff7, 0.95).setDepth(5);
      this.tweens.add({
        targets: sparkle,
        x: this.light.x + Math.cos(angle) * (36 + i * 4),
        y: this.light.y + Math.sin(angle) * (28 + i * 3),
        alpha: 0,
        scale: 0.35,
        duration: 850 + i * 45,
        ease: 'Sine.easeOut',
...
export function WorldHost({
  ...
  pendingSequence,
  onSequenceComplete,
}: LandingMeadowState & { onSequenceComplete: (sequence: 'first-light') => void }) {
  const host = useRef<HTMLDivElement>(null);
  const scene = useRef<LandingMeadowScene | null>(null);
  const complete = useRef(onSequenceComplete);
  complete.current = onSequenceComplete;

  useEffect(() => {
    if (!host.current) return;
    const landing = new LandingMeadowScene(
      { firstLightRevealed, pendingSequence },
      sequence => complete.current(sequence),
    );
    scene.current = landing;
  ...
  useEffect(() => {
    scene.current?.setWorldState({ firstLightRevealed, pendingSequence });
  }, [firstLightRevealed, pendingSequence]);
